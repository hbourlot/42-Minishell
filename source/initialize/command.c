...
/*   Updated: 2025/02/06 18:22:13 by joralves         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	split_command_input(t_shell *data, const char *delimiters[])
{
	...
	data->readline_splitted = split_by_multiple_tokens(data->readline,
			delimiters);
...
static int	create_command_list(t_shell *data, const char *delimiters[])
{
	...
	while (data->readline_splitted[i])
	{
		...
		if (match_result == CMP_OK)
		{
			...
			token_type = NO_TOKEN;
		if (add_command(&data->command, data->readline_splitted[i++], data,
				token_type) < 0)
			return (ERROR);
	}
